...
class ServerlessPlugin {
  ...
  compileRulesToEventBridge() {
    ...
    const rulesTemplate = targetAccounts.map(account => {
      ...
      const alphaNumericResourceRuleName = ruleName.replace(/[^a-z0-9+]+/gi, '');
      const formattedRuleName = ruleName.replace(/[^a-z0-9+-]+/gi, '');
      const region = account.region || 'eu-central-1';

      return {
        [alphaNumericResourceRuleName]: {
          "Type" : "AWS::Events::Rule",
          "Properties" : {
            "Name" : formattedRuleName,
